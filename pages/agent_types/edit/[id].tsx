...
export default function EditAgentTypesPage() {
  ...
  const handleUpdate = async (e: any) => {
    e.preventDefault();
    const { error } = await supabase.from("agent_types").update({ name, description, status }).eq("id", id);
    if (!error) router.push(`/agent_types/{id}`);
  };

  return (
    <div className="p-6 max-w-xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">✏️ 编辑 Agent Types</h1>
