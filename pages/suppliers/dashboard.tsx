// 📁 文件路径：panel-a/pages/supplier/dashboard.tsx
...
export default function SupplierDashboard() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      setEmail(user.email);
      const { data, error } = await supabase.from('supplier_profile').select('company_name').eq('user_id', user.id).single();
  ...
  return (
    <div className="p-8">
      <h2 className="text-xl font-bold mb-2">Welcome to Supplier Dashboard</h2>
