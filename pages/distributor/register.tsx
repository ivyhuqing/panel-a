// 📁 文件路径：panel-a/pages/distributor/register.tsx
...
export default function DistributorRegister() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (userId) {
      const { error: profileError } = await supabase.from('distributor_profile').insert([
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-blue-50">
      <form onSubmit={handleRegister} className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-6 text-center">Distributor Registration</h2>
