...
export default function EditClientsPage() {
  ...
  const handleUpdate = async (e: any) => {
    e.preventDefault();
    const { error } = await supabase.from("clients").update({ name, description, status }).eq("id", id);
    if (!error) router.push(`/clients/{id}`);
  };

  return (
    <div className="p-6 max-w-xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">✏️ 编辑 Clients</h1>
